...
div_publications = tmsoup.find_all('div', attrs={
    'id': 'text-publs'})  # locates the section in the html code where the publs are stored
publs = div_publications[0].find_all('p')  # breaks the section into pagragraphs (each contains a publ)
p_textrelations = tmsoup.find_all('p',
                                  attrs={'id': 'texrelations'})  # locates the section in the html where the links are
...
if len(p_textrelations) > 0:
    ...
    for link in links:  # finds the one that directs to papyri.info
        if 'DDbDP' in link.text:
            papinfo = link['href']  # stores the url in a variable
else:
    papinfo = None
...
if len(publications) > 2:
    # checks if the text of the last item of the list is repeated in the previous item
    if publications[-1] in publications[-2]:
        # if so, it removes it
        publications[-2] = publications[-2].replace(publications[-1], '')
